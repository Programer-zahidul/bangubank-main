...
if (isset($_POST["submit"])) {
  ...
  if (isset($_SESSION["user"]["email"])) {
    ...
    try {
      Helpers::writeFile($file_path, $deposite);
      // $deposite_file = fopen("../data/balance.txt", "a") or die("Could not oper the file.");
      // fwrite($deposite_file, $deposite);
      // fclose($deposite_file);
      $current_balance = $new_balance;
      $success_msg = " Balance Deposite Successfully.";
    } catch (Exception $e) {
      $error_msg = "Could Not open a file." . $e->getMessage();
    }
  } else {
        ...
        <div class="bg-white rounded-lg p-2">
          ...
          <?php if ($success_msg != "") { ?>
            <div
              ...
              role="alert">
              <span class="font-medium">Success alert!</span> <?php echo $success_msg; ?>
            </div>
          <?php } ?>
          <?php if ($error_msg != "") { ?>
            <div
              class="p-4 mb-4 text-sm text-center text-red-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400"
              role="alert">
              <span class="font-medium">Danger alert!</span><?php echo $error_msg; ?>
            </div>
          <?php } ?>
